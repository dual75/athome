...
class MqttBrokerTest(common.AsyncTest):
    """Test hbmqtt broker setup"""

    def test_run_broker(self):
        """broker is correctly started and stopped"""
        
        self.loop.run_until_complete(mqtt.run_broker(BROKER_CONFIG))
        self.assertIsNotNone(mqtt.broker)
        self.loop.run_until_complete(mqtt.stop_broker())
        self.assertIsNone(mqtt.broker)


class MqttClientTest(common.AsyncTest):
    """Test mqtt client"""
